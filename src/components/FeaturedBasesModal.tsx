...
interface FeaturedBasesModalProps {
  isOpen: boolean;
  onClose: () => void;
}

// Extended list of featured bases
const allFeaturedBases: Base[] = [
...
export default function FeaturedBasesModal({
  isOpen,
  onClose,
}: FeaturedBasesModalProps) {
  return (
      ...
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content maxW="1200px" maxH="80vh" overflowY="auto">
            <Dialog.Header>
              <Dialog.Title>Featured Bases</Dialog.Title>
            </Dialog.Header>
            <Dialog.Body>
              <Box py={4}>
                <SimpleGrid columns={{ base: 1, md: 2, lg: 3, xl: 4 }} gap={4}>
                  {allFeaturedBases.map((base) => (
                    <Card.Root
                      key={base.id}
                      variant="subtle"
                      cursor="pointer"
